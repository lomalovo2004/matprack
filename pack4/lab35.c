...
EXIT_CODE infix_to_postfix(const char *infix, char **postfix, int* error)
{
...
EXIT_CODE solve_expression(char *postfix, int *result, int* error)
{
    int number = 0;
    int number_1 = 0;
    int number_2 = 0;
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        else
        {
            ...
            switch (postfix[i])
            {
                ...
                case '^':
                    if (pow(number_1, number_2) > INT_MAX)
                    {
                        *error = OVERFLOWED;
                        return OVERFLOWED;
                    }
                    ...
                    {
                        push(stack, pow(number_1, number_2));
                    }
                    break;
...
char *error_file(char *filename)
{
    char *prefix = "error_";
    char *output = NULL;
    output = (char *) malloc(strlen(prefix) + strlen(filename) + 1);
    if (!output)
    {
        return NULL;
    }
...
EXIT_CODE validate_infix_expression(const char *infix, int* error)
{
...
void print_error_in_file(FILE* output_file, int error, int line_of_expression, char* string)
{
    ...
    switch (error)
    {
        ...
        case MATH_ERROR:
            fprintf(output_file, "mathematic error\n");
            break;
...
EXIT_CODE file_works(FILE *input_file, FILE *output_file)
{
    int symbol = fgetc(input_file);
    int capacity = 10;
    int error;
    int result;
    EXIT_CODE st_solve_expression;
    EXIT_CODE st_postfix_expression;
    ...
    while (flag && !feof(input_file))
    {
        ...
        if (length > 0)
        {
            ...
            EXIT_CODE st_validate_infix_expression = validate_infix_expression(string, &error);
                    ...
                    switch (st_postfix_expression)
                    {
                        ...
                        case OK:
                            ...
                            switch (st_solve_expression)
                            {
                                ...
                                case MATH_ERROR:
                                    print_error_in_file(output_file, error, line_of_expression, string);
                                    printf("Wrong expression, mathematic error\n");
                                    break;
...
EXIT_CODE all_functions(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        EXIT_CODE st = file_works(input_file, output_file);
...
int main(int argc, char *argv[])
{
    if (argc < 2)
    {
        printf("Use: ./lab35 lab35_input1.txt lab35_input2.txt lab35_input3.txt\n");
        return 0;
    }

    EXIT_CODE st = all_functions(argc, argv);
    if (st != OK)
    {
        print_errors(st);
    }
